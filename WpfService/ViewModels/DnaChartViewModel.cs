...
{
    public class DnaChartViewModel : ChartViewModelBase, IDisposable
    {
        private readonly ShellViewModel _shell;
        private readonly DnaInputDialogViewModel _input = new();
        ...
        public DnaChartViewModel(ShellViewModel shell)
        {
            _shell = shell;
            ...
            _table.Columns.Add("A230", typeof(double));
            _table.Columns.Add("A260/A280", typeof(double));
            _table.Columns.Add("A260/A230", typeof(double));
            NormalChart();
        ...
        public async void Load(string experimentType, int id = -1)
        {
            ...
            AssayType = Assays.Instance.GetDnaAssay(ExperimentType);
            IsInputVisible = ExperimentType == "CustomFactor";
            NormalChart();
            ...
            if (response.Data is JObject obj)
            {
                try
                {
                    ...
                    foreach (var measurement in measurements)
                    {
                        ...
                        double a260 = (double)measurement.MeasurementData["peak_abs"];
                        double a280 = (double)measurement.MeasurementData["main_abs"];
                        double a230 = (double)measurement.MeasurementData["secondary_abs"];
                        double a260A280 = a260 / a280;
                        double a260A230 = a260 / a230;
        ...
        private async Task ShowParameterDialogAsync()
        {
            ...
            _parameter.ParamterData = DnaParams.Instance;

            var result = ShowDialog(_parameter, "Parameter Settings");
            if (result == MessageBoxResult.OK)
            {
                var param = await ApiClient.SetDnaParams();
                if (!param.Result)
                {
                    IsLoading = false;
                    Dialog.Error(response.ErrMsg);
        ...
        {
            decimal customFactor = _input.CustomFactor;
            ...
            if (result == MessageBoxResult.OK)
                ...
                _input.CustomFactor = customFactor;
        }

        private async Task MeasurementAsync()
        {
            IsLoading = true;
            string customFactor = IsInputVisible ? _input.CustomFactor.ToString() : string.Empty;
            var response = await ApiClient.Sample(ExperimentType, SampleName, factor: customFactor);
            ...
            try
            {
                if (response.Data is JObject obj)
                {
                    ...
                    double a260 = Convert.ToDouble(datas["peakAbs"]);
                    double a280 = Convert.ToDouble(datas["mainAbs"]);
                    double a230 = Convert.ToDouble(datas["secondaryAbs"]);
        ...
        private void HideChart()
        {
            ...
            VisibleColumns = new List<string> { "No", "Sample", "Conc", "A260/A280", "A260/A230", "A260", "A280", "A230" };
        ...
        private void NormalChart()
        {
            ...
            VisibleColumns = new List<string> { "No", "Conc", "A260/A280", "A260/A230" };

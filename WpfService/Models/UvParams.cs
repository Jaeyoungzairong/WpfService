...
{
    public class UvParams
    {
        public static UvParams Instance { get; } = new UvParams();
        private UvParams() { }
        ...
        public bool SetData(object obj)
        {
            try
            {
                ...
                foreach (var prop in typeof(UvParams).GetProperties(BindingFlags.Public | BindingFlags.Instance))
                {
        ...
        {
            var props = typeof(UvParams).GetProperties(BindingFlags.Public | BindingFlags.Instance);
            var list = props
                .Select(p => new
                {
                    Key = p.Name.ToLower(),

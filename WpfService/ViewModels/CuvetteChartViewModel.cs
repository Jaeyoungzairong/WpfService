...
{
    public class CuvetteChartViewModel : ChartViewModelBase, IDisposable
    {
        private readonly ShellViewModel _shell;
        private readonly CuvetteInputDialogViewModel _input = new();
        ...
        public bool IsChartVisible{ get => GetValue<bool>(); set => SetValue(value); }
        ...
        public bool IsInputVisible 
        {
            ...
            {
                if (SetValue(value))
                    RaisePropertyChanged(nameof(ButtonCount));
            }
        }
        public int ButtonCount => IsInputVisible ? 4 : 3;

        public string InputText { get => GetValue<string>(); set => SetValue(value); }

        public int? XMin { get => GetValue<int?>(); set => SetValue(value); }

        public int? XMax { get => GetValue<int?>(); set => SetValue(value); }
        ...
        public CuvetteChartViewModel(ShellViewModel shell)
        {
            _shell = shell;
            ...
            _table.Columns.Add("Wavelength", typeof(double));
            _table.Columns.Add("Abs", typeof(double));
            NormalChart();
        ...
        public async void Load(string experimentType, int id = -1)
        {
            ...
            AssayType = Assays.Instance.GetCuvetteAssay(ExperimentType);
            IsChartVisible = ExperimentType == "Spectrum" || ExperimentType == "Kinetic";
            InputText = (ExperimentType == "Spectrum" || ExperimentType == "EndPoint") ? "Wavelength" : "Input";
            IsInputVisible = ExperimentType != "OD600";

            if (IsChartVisible)
            {
                AssayType.RangeFrom = 200;
                AssayType.RangeTo = 800;
                NormalChart();
            }
            else
            {
            ...
            if (IsInputVisible)
                AssayType.PeakWL = 600;


            XMin = (IsDataView && IsChartVisible) ? null : AssayType.RangeFrom;
            XMax = (IsDataView && IsChartVisible) ? null : AssayType.RangeTo;
            _input.RangeFrom = AssayType.RangeFrom;
            _input.RangeTo = AssayType.RangeTo;
            _input.Wavelength = AssayType.PeakWL;
            _input.IsRange = IsChartVisible;
            ...
            if (response.Data is JObject obj)
            {
                try
                {
                    ...
                    foreach (var measurement in measurements)
                    {
                        ...
                        double wavelength = (double)measurement.MeasurementData["wavelength"];
                        double abs = (double)measurement.MeasurementData["abs"];
        ...
        private async Task ShowParameterDialogAsync()
        {
            ...
            _parameter.ReadOnlyCategories = new List<string> { "1. Common" };
            _parameter.ParamterData = CuvetteParams.Instance;

            var result = ShowDialog(_parameter, "Parameter Settings");
            if (result == MessageBoxResult.OK)
            {
                var param = await ApiClient.SetCuvetteParams();
                if (!param.Result)
                {
                    IsLoading = false;
                    Dialog.Error(response.ErrMsg);
        ...
        private async Task BlankAsync()
        {
            ...
            IsBlank = true;
            IsInput = !IsInputVisible;
            _blankTimer.Start();
        }

        private void ShowInputDialog()
        {
            int rangeFrom = (int)_input.RangeFrom;
            int rangeTo = (int)_input.RangeTo;
            int wavelength = (int)_input.Wavelength;
            _input.IsReadOnly = IsMeasurement || ExperimentType == "OD600";
            var result = ShowDialog(_input, "Input", MessageBoxButton.OK);

            if (result == MessageBoxResult.OK)
            {
                if (ExperimentType == "Spectrum")
                {
                    AssayType.RangeFrom = (int)_input.RangeFrom;
                    AssayType.RangeTo = (int)_input.RangeTo;
                    XMin = AssayType.RangeFrom;
                    XMax = AssayType.RangeTo;
                }
                IsInput = true;
            }
            else
            {
                _input.RangeFrom = rangeFrom;
                _input.RangeTo = rangeTo;
                _input.Wavelength = wavelength;
            }
        }
        ...
        {
            IsLoading = true;
            var response = await ApiClient.Sample(
                ExperimentType, 
                SampleName, 
                _input.RangeFrom.ToString(),
                _input.RangeTo.ToString(), 
                _input.Wavelength.ToString());
            ...
            try
            {
                ...
                {
                    var datas = obj.ToObject<Dictionary<string, object>>();
                    int wavelength = Convert.ToInt32(datas["peakWL"]);
                    double a600 = Convert.ToDouble(datas["peakAbs"]);
        ...
        private void ApplySelectionToChart()
        {
            ...
            else
            {
                ...
                {
                    var s = _allSeries.FirstOrDefault(x => x.Id == active[i]);
                    if (s == null) continue;
        ...
        private void HideChart()
        {
            ...
            VisibleColumns = new List<string> { "No", "Sample", "Wavelength", "Abs" };
        ...
        private void NormalChart()
        {
            ...
            VisibleColumns = new List<string> { "No", "Wavelength", "Abs" };
        ...
        private void FullChart()
        {
            ...
            VisibleColumns = new List<string> { "No", "Abs" };
